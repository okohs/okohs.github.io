(function(global){
    ...
    this.pos = {x : 0, y : 0};
        ...
        if (fusen_util.getDevice() == "mobile") {
						this.canvas.append('<div id="'+this.id+'" class="p-husen theme-husen--'+color+'">'+'<img src="http://www.paper-glasses.com/api/twipi/'+fusen_util.htmlEscape(text)+'"></div>');
        } else {
						this.canvas.append('<div id="'+this.id+'" class="p-husen theme-husen--'+color+'">'+'<img src="http://www.paper-glasses.com/api/twipi/'+fusen_util.htmlEscape(text)+'" alt="profile image" width="55" height="55">'+'<div class="p-husen__cross">×</div></div>');
        }
	...
	Fusen.prototype.setPos = function(x, y) {
		...
		var self = this;
		self.pos.x = x+OFFSETX;
		self.pos.y = y+OFFSETY;
